...
Bill of Materials
Build Guide — 13 Steps from Instructables
...
> 🔗 **<https://www.instructables.com/Autoguided-Telescope/>**
🎯 Project Goals
...
🎥 Watch: ▶️ Building the Rotating Base
---
...
🎥 Watch: ▶️ Building the X-Axis Rotation
---
🔬 Step 4 — Lenses
...
🎥 Watch: ▶️ Telescope Body Assembly
---
...
Three signalling pins synchronise the active mode:
Pins D9 / D8 on the Remote Arduino map to D10 / D11 on the Motor Arduino, advertising the current mode.
...
The firmware ingests target RA/Dec + observer Lat/Lon + current UTC and solves the spherical PZS triangle (see Mathematical Foundation). A simplifying assumption is made: distant stars' RA/Dec drift is negligible over short sessions, so precession/nutation/aberration are intentionally omitted to keep the on-board maths fast.
---
...
a. Online Mode — receive Alt/Az over Serial, drive the steppers using the AccelStepper library with acceleration/deceleration profiles.
b. Offline Mode — use the hardcoded star catalog plus on-board spherical maths.
c. Joystick Mode — direct manual control with adjustable sensitivity thresholds.
...
🎥 Watch: ▶️ Final Test & First Light
---
📂 Repository Structure
```
AK-SCOPE/
...
🚀 Getting Started
Prerequisites
Hardware — completed assembly per Build Guide.
Software
...
```
The GUI lists every available COM port — pick the one assigned to Arduino #2 (motors).
Choose a celestial object from the catalog.
Click Search — Selenium fetches the live Alt/Az from Stellarium Web.
Coordinates are streamed to the telescope and the steppers begin slewing.
🛰️ Offline Mode (no PC)
Power up — wait for the calibration sequence (altitude axis seeks the home switch, then goes to 0°).
Press Button #2 to cycle to Offline.
Scroll the joystick through the 20-star catalog; press to confirm — the telescope slews automatically.
🕹️ Manual Mode
Press Button #2 to cycle to Manual.
Push the joystick — telescope slews at a speed proportional to deflection.
---
🔌 Pin Assignments & Wiring
...
See the open issues for the full backlog of proposed features and known bugs.
---
🤝 Contributing
...
Fork the project
Create your feature branch (`git checkout -b feature/MyAmazingFeature`)
Commit your changes (`git commit -m 'Add MyAmazingFeature'`)
Push to the branch (`git push origin feature/MyAmazingFeature`)
Open a Pull Request
Good first issues
...
🐙 GitHub: @OussamaAKHAIL
🔗 Project Link: https://github.com/OussamaAKHAIL/AK-SCOPE
📰 Instructables: https://www.instructables.com/Autoguided-Telescope/
For questions, feature ideas, or build help, please open an issue — that way the answer benefits the whole community.
---
<div align="center">
